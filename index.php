...
<style type="text/css">
  ...
  input[type=text], input[type=password] { display: block; border: 1px solid #CCC; padding: 2px 4px; }
  input[type=submit] { margin-top: 15px; font-size: 18px; padding: 2px 6px; }
...
<form action="https://api.samurai.feefighters.com/v1/payment_methods" method="POST">
  <fieldset>
    ...
    <input id="credit_card_first_name" name="credit_card[first_name]" type="text" value="<?php echo $samurai_payment_method ? $samurai_payment_method->getFirstName() : null; ?>" />
    ...
    <select id="credit_card_card_type" name="credit_card[card_type]">
      <option></option>
      <option value="visa" <?php echo $samurai_payment_method && $samurai_payment_method->getCardType() == 'visa' ? 'selected="selected"' : null; ?>>Visa</option>
      <option value="master"<?php echo $samurai_payment_method && $samurai_payment_method->getCardType() == 'master' ? 'selected="selected"' : null; ?>>MasterCard</option>
    </select>
